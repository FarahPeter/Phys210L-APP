...
def DisplayImage(x):
    ...
    resized_image= im.resize((700,700), Image.ANTIALIAS)
        ...
        try:
            ...
            if (whoCalled=="roundError"):
                ...
                if ( a[1]>1 or a[1]<0):
                    ...
                    return()
                res= primecheck(int(a[0]),a[1])
                #res=[0,0]
                #t1 = threading.Thread(target=primecheck, args=(int(a[0]),a[1],))
                #t1.start()
                #t1.join()
                if (res[0]==True):
                    output['text']=(str(int(a[0]))+" is a prime number with certainty:"+str(res[1]))
                elif (res[0]==False):
                    output['text']=(str(int(a[0]))+" is a composite number with certainty: 1")
...
helpmenu.add_command(label="Exit",command=lambda: master.destroy())
...
aboutmenu.add_command(label="Version",command=lambda: popupmsg("Last Updated 17/6/2022\n \n V1.1","Version","200x120"))
